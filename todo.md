...
## Phase 66 - 입국심사 체크리스트 + PDF 다운로드 + JWT 수정 (2026-04-11)
- [x] DB 스키마: travelDocuments 테이블 추가 (여권/항공권/바우처 등록 테이블) 타입, 만료일)
- [x] tRPC 프로시저: travelDocs.list, travelDocs.upsert, travelDocs.delete
- [x] 마이페이지 입국심사 체크리스트 UI (여권/항공권/호텔바우처 등록 상태 한눈에 확인)
- [x] 파일 업로드 → S3 저장 → URL 반환 기능
- [x] PDF 다운로드 기능 (호텔 바우처/항공권 PDF 생성 또는 등록된 파일 다운로드)
- [x] JWT 세션 문제 수정: context.ts가 openId 기반으로 사용자 조회하도록 수정
