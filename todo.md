...
## Phase 33 - 기능 32개 순차 구현

### 공지사항 (Notices)
- [x] 1. 공지사항 조회수 카운터 - 상세 페이지 진입 시 조회수 증가, 목록에 조회수 표시
- [x] 2. 공지사항 이전/다음 네비게이션 - 상세 페이지 하단 이전 글 / 다음 글 링크
- [x] 3. 공지사항 다국어 탭 - 상세 페이지에서 번역 언어 탭 전환
- [x] 4. 공지사항 즐겨찾기 - 로그인 사용자가 공지 북마크 저장

### 플랜 / 투자 (Plans)
- [x] 5. 플랜 상세 페이지 개선 - 수익률 차트, 투자 기간 시각화, 유사 플랜 추천
- [x] 6. 플랜 비교 기능 - 2~3개 플랜 나란히 비교 페이지
- [x] 7. 플랜 즐겨찾기 정렬 - 즐겨찾기 페이지 카테고리/수익률 정렬 옵션
- [x] 8. 플랜 리뷰/별점 - 사용자가 투자한 플랜에 별점 및 한줄 평 작성

### 사용자 대시보드 (User Dashboard)
- [x] 9. 투자 포트폴리오 차트 - 보유 플랜별 비중 파이 차트
- [x] 10. 수익 히스토리 그래프 - 월별/주별 수익 추이 라인 차트
- [x] 11. 내 활동 타임라인 - 투자, 투표, 에어드랍 참여 등 활동 이력
- [x] 12. 알림 센터 - 공지, 투표 마감, 에어드랍 등 개인화 알림 목록 페이지

### 레퍼럴 / MLM
- [x] 13. 레퍼럴 링크 공유 버튼 + QR - 카카오톡/텔레그램/트위터 공유 + QR 코드
- [x] 14. 레퍼럴 순위 리더보드 - 상위 레퍼러 공개 랭킹 페이지
- [x] 15. 레퍼럴 수익 예측기 - 하위 회원 수 입력 시 예상 수익 계산 UI

### 에어드랍 (Airdrop)
- [x] 16. 에어드랍 카운트다운 타이머 - 마감 임박 에어드랍에 실시간 카운트다운
- [x] 17. 에어드랍 참여 현황 바 - 목표 인원 대비 현재 참여율 프로그레스 바
- [x] 18. 에어드랍 히스토리 - 사용자가 참여한 에어드랍 내역 및 수령 상태

### 리스팅 / 투표 (Listing / Vote)
- [x] 19. 투표 결과 실시간 차트 - 진행 중인 투표의 찬반 비율 실시간 바 차트
- [x] 20. 리스팅 신청 진행 상태 트래커 - 신청 → 검토 → 투표 → 완료 단계 시각화
- [x] 21. 리스팅 신청 임시 저장 - 작성 중인 신청서 로컬 임시 저장

### SNS / 트렌딩
- [x] 22. SNS 피드 무한 스크롤 - 현재 페이지네이션을 무한 스크롤로 전환
- [x] 23. 인플루언서 팔로우 기능 - 특정 인플루언서 즐겨찾기 등록 및 필터
- [x] 24. 트렌딩 알림 설정 - 사용자별 관심 코인 트렌딩 알림 구독 설정

### 어드민 백오피스
- [x] 25. 어드민 대시보드 실시간 지표 - 오늘 신규 가입, 투자, 투표 수 실시간 카운터
- [x] 26. 사용자 일괄 메시지 발송 - 조건 필터로 대상 선택 후 텔레그램/공지 일괄 발송
- [x] 27. 어드민 플랜 수익률 일괄 수정 - 여러 플랜 수익률 한 화면 인라인 편집
- [x] 28. 리워드 자동 지급 스케줄러 - 설정 주기마다 자동 리워드 계산 및 지급

### UX / 성능
- [x] 29. 다크/라이트 모드 전환 버튼 - 헤더에 테마 토글 버튼 추가
...
## 프로젝트 문서화
- [x] 개발자 전달용 AlphaBag 종합 기능 소개서 중국어 간체 번역 및 PDF 제작

## 포인트·노드 경제 시스템 및 운영 바이블
- [x] 한국어·중국어·영어 운영 바이블 원고 및 100페이지 이상 PDF 제작 (한국어 146p, 중국어 135p, 영어 152p)
- [x] 투자금의 USDT·포인트 복합 결제 비율과 포인트 적립 규칙 확정 (USDT 70~95%, ABP 5~30%, 합계 100% 강제)
- [x] 포인트 원장, 잔액, 거래 주문, 체결, 에스크로, 지갑 출금 데이터 모델 설계 (8개 신규 테이블·3개 안전 마이그레이션 적용)
- [x] 포인트 전송·P2P 매도 주문·USDT 결제 준비·정산 보안 정책 수립 (KYC·서명 지갑·체인별 활성화·원자적 EVM 에스크로)
- [x] 노드 등급, 권한, 보상 및 포인트 연계 정책 설계 (버전형 노드 혜택 정책·적립·결제 한도·P2P 수수료 할인)
- [x] 포인트·노드 사용자 화면 및 관리자 운영 화면 구현 (ABP Market·복합 결제 탭·체인 구성·노드/결제 정책 관리)
- [x] 거래·정산·권한 로직 Vitest 테스트 및 빌드 검증 (신규 테스트 10개·TypeScript·Solidity 컴파일·프로덕션 빌드 통과)
- [ ] 메인넷 계약 감사·멀티시그 설정·RPC 인덱서·KYC/AML 운영 승인 후 실자금 기능 활성화

## AlphaBag A포인트·B포인트 이중 원장 제도
- [x] A포인트·B포인트 경제 정책과 상태 전이 정의 (내부 원장형, 1 A=1 B=1 USDT 기준, B 출금 준비금 연계)
- [x] 프로젝트 실패 회원가입·레그 이동 A포인트 지급 신청·이중 관리자 승인 정책 구현
- [x] A포인트 회원 간 내부 전송 및 멱등 원장 이력 구현
- [x] 투자 결제 시 USDT 70~95% + A포인트 5~30% 사용·A 예약 규칙 구현
- [x] USDT TxHash 확인 후 투자 완료 시 사용 A포인트의 B포인트 1:1 전환·귀속 규칙 구현
- [x] 미연결 프로젝트 100% USDT 매출 시 A포인트 5~30%를 B포인트로 전환하는 증빙·승인 흐름 구현
- [x] B포인트 USDT 출금 신청·준비금 커밋·관리자 승인·TxHash 완료 처리 구현
- [x] A/B 포인트 사용자 지갑·전송·투자 사용·전환·출금 화면 구현 (`/ab-points`)
- [x] A/B 포인트 관리자 지급·투자 확인·전환·준비금·출금 관리 화면 구현 (`/admin/ab-points`)
- [x] 경제 규칙·인증 경계 테스트 19개, TypeScript, 프로덕션 빌드, 데스크톱·모바일 화면 검증

## A/B 포인트 운영 배포 및 운영 바이블
- [x] 배포 소스에 없는 `dist/`를 복사하는 사용자 Dockerfile 실패 수정 (불필요한 루트 Dockerfile 제거, 플랫폼 기본 인이미지 빌드 사용)
- [x] A/B 포인트 내부 원장 제도를 반영한 한국어 운영 바이블 작성 및 PDF 제작 (43페이지, 40개 SOP)
- [x] A/B 포인트 코드·테스트·프로덕션 빌드 재검증 (TypeScript, 19개 테스트, 빌드 성공)
- [x] AWS Lightsail 운영 DB에 A/B 포인트 마이그레이션 적용 (0045~0047, 사전 백업 완료)
- [x] alphabag.net Docker 컨테이너 재빌드 및 재배포
- [x] alphabag.net `/ab-points`·API·컨테이너 상태 최종 검증 (HTTP 200, restart 0, 치명 로그·브라우저 오류 없음)
