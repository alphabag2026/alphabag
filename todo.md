...
### 공지사항 (Notices)
- [ ] 1. 공지사항 조회수 카운터 - 상세 페이지 진입 시 조회수 증가, 목록에 조회수 표시
- [ ] 2. 공지사항 이전/다음 네비게이션 - 상세 페이지 하단 이전 글 / 다음 글 링크
- [ ] 3. 공지사항 다국어 탭 - 상세 페이지에서 번역 언어 탭 전환
- [ ] 4. 공지사항 즐겨찾기 - 로그인 사용자가 공지 북마크 저장

### 플랜 / 투자 (Plans)
- [ ] 5. 플랜 상세 페이지 개선 - 수익률 차트, 투자 기간 시각화, 유사 플랜 추천
- [ ] 6. 플랜 비교 기능 - 2~3개 플랜 나란히 비교 페이지
- [ ] 7. 플랜 즐겨찾기 정렬 - 즐겨찾기 페이지 카테고리/수익률 정렬 옵션
- [ ] 8. 플랜 리뷰/별점 - 사용자가 투자한 플랜에 별점 및 한줄 평 작성

### 사용자 대시보드 (User Dashboard)
- [ ] 9. 투자 포트폴리오 차트 - 보유 플랜별 비중 파이 차트
- [ ] 10. 수익 히스토리 그래프 - 월별/주별 수익 추이 라인 차트
- [ ] 11. 내 활동 타임라인 - 투자, 투표, 에어드랍 참여 등 활동 이력
- [ ] 12. 알림 센터 - 공지, 투표 마감, 에어드랍 등 개인화 알림 목록 페이지

### 레퍼럴 / MLM
- [ ] 13. 레퍼럴 링크 공유 버튼 + QR - 카카오톡/텔레그램/트위터 공유 + QR 코드
- [ ] 14. 레퍼럴 순위 리더보드 - 상위 레퍼러 공개 랭킹 페이지
- [ ] 15. 레퍼럴 수익 예측기 - 하위 회원 수 입력 시 예상 수익 계산 UI

### 에어드랍 (Airdrop)
- [ ] 16. 에어드랍 카운트다운 타이머 - 마감 임박 에어드랍에 실시간 카운트다운
- [ ] 17. 에어드랍 참여 현황 바 - 목표 인원 대비 현재 참여율 프로그레스 바
- [ ] 18. 에어드랍 히스토리 - 사용자가 참여한 에어드랍 내역 및 수령 상태

### 리스팅 / 투표 (Listing / Vote)
- [ ] 19. 투표 결과 실시간 차트 - 진행 중인 투표의 찬반 비율 실시간 바 차트
- [ ] 20. 리스팅 신청 진행 상태 트래커 - 신청 → 검토 → 투표 → 완료 단계 시각화
- [ ] 21. 리스팅 신청 임시 저장 - 작성 중인 신청서 로컬 임시 저장

### SNS / 트렌딩
- [ ] 22. SNS 피드 무한 스크롤 - 현재 페이지네이션을 무한 스크롤로 전환
- [ ] 23. 인플루언서 팔로우 기능 - 특정 인플루언서 즐겨찾기 등록 및 필터
- [ ] 24. 트렌딩 알림 설정 - 사용자별 관심 코인 트렌딩 알림 구독 설정

### 어드민 백오피스
- [ ] 25. 어드민 대시보드 실시간 지표 - 오늘 신규 가입, 투자, 투표 수 실시간 카운터
- [ ] 26. 사용자 일괄 메시지 발송 - 조건 필터로 대상 선택 후 텔레그램/공지 일괄 발송
- [ ] 27. 어드민 플랜 수익률 일괄 수정 - 여러 플랜 수익률 한 화면 인라인 편집
- [ ] 28. 리워드 자동 지급 스케줄러 - 설정 주기마다 자동 리워드 계산 및 지급

### UX / 성능
- [ ] 29. 다크/라이트 모드 전환 버튼 - 헤더에 테마 토글 버튼 추가
- [ ] 30. 모바일 하단 탭 네비게이션 - 모바일 홈/플랜/에어드랍/마이페이지 하단 고정 탭
- [ ] 31. 페이지 로딩 스켈레톤 개선 - 주요 페이지에 스켈레톤 UI 적용
- [ ] 32. 검색 자동완성 - 메인 검색창에 플랜명 자동완성 드롭다운
