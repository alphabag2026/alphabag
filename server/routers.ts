...
Return ONLY valid JSON.`;
  ...
  travelDocs: router({
    ...
    list: protectedProcedure.query(async ({ ctx }) => {
      const database = await getDb();
      if (!database) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
      const { travelDocuments } = await import("../drizzle/schema");
      const { eq, desc } = await import("drizzle-orm");
      return database.select().from(travelDocuments)
        .where(eq(travelDocuments.userId, ctx.user.id))
        .orderBy(desc(travelDocuments.createdAt));
    }),
    // 여행 서류 등록/수정
    upsert: protectedProcedure.input(z.object({
      id: z.number().optional(),
      docType: z.enum(["passport", "flight_ticket", "hotel_voucher", "visa", "travel_insurance", "other"]),
      title: z.string().min(1).max(200),
      fileUrl: z.string().optional(),
      ...
      if (input.id) {
        // 수정: 본인 서류만 수정 가능
        await database.update(travelDocuments).set({
          docType: input.docType,
          title: input.title,
          fileUrl: input.fileUrl ?? null,
          expiryDate: input.expiryDate ?? null,
          note: input.note ?? null,
        }).where(and(eq(travelDocuments.id, input.id), eq(travelDocuments.userId, ctx.user.id)));
        return { success: true };
      } else {
        // 신규 등록
        const result = await database.insert(travelDocuments).values({
          userId: ctx.user.id,
          docType: input.docType,
          title: input.title,
          fileUrl: input.fileUrl ?? null,
    ...
    delete: protectedProcedure.input(z.object({ id: z.number() })).mutation(async ({ ctx, input }) => {
      ...
      const { travelDocuments } = await import("../drizzle/schema");
      const { eq, and } = await import("drizzle-orm");
      await database.delete(travelDocuments)
        .where(and(eq(travelDocuments.id, input.id), eq(travelDocuments.userId, ctx.user.id)));
      return { success: true };
    }),
    // 파일 업로드 URL 생성 (S3)
    uploadFile: protectedProcedure.input(z.object({
      ...
      const key = `travel-docs/${ctx.user.id}/${Date.now()}-${Math.random().toString(36).slice(2)}.${ext}`;
