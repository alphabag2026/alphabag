      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-black text-orange-400 flex items-center gap-2">
              <span className="text-2xl">⭐</span>
              Influencer
            </h1>
          </div>
          <p className="text-gray-400 text-sm">인플루언서 추천 · 트렌딩 · 소셜 검증</p>
        </div>
            ...
            {sorted.map((plan: any) => {
              ...
              return (
                <div
                  ...
                  className="relative bg-[#0d0d0d] border border-orange-500/30 rounded-xl overflow-hidden cursor-pointer transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-orange-500/40 group"
                >
                  <div className="relative h-36 overflow-hidden">
                    {plan.logoUrl ? (
                      ...
                      <div className="w-full h-full bg-gradient-to-br from-orange-900/40 to-orange-600/20 flex items-center justify-center transition-transform duration-500 group-hover:scale-110">
                        <span className="text-5xl font-black opacity-30 text-orange-400">{plan.name.charAt(0)}</span>
                      </div>
                    )}
                    <div className="absolute inset-0 bg-gradient-to-t from-orange-900/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                  </div>
                  <div className="p-4">
                    ...
                    {badges.length > 0 && (
                      <div className="flex flex-wrap gap-1 mb-3">
                        {badges.slice(0, 3).map((b: string, i: number) => (
                          <span key={i} className="text-[10px] px-2 py-0.5 rounded-full border bg-orange-500/20 text-orange-300 border-orange-500/30">{b}</span>
                        ))}
                      </div>
                    )}
                    <div className="mb-3">
                      <div className="text-2xl font-bold text-orange-400">{Number(plan.dailyRate).toFixed(2)}%</div>
                    ...
                    <button className="w-full py-2 rounded-lg text-xs font-semibold transition-all duration-200 border bg-orange-500/20 text-orange-300 border-orange-500/30 hover:bg-orange-500/30">
                      View Details →
                    </button>
                  </div>
                </div>
