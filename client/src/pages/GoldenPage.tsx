      ...
      <div className="relative bg-card border border-amber-200/60 rounded-xl overflow-hidden cursor-pointer transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:shadow-amber-200/60 hover:border-amber-400/80 group flex flex-col">
        ...
        <div className="relative h-40 overflow-hidden bg-gray-950">
          {plan.logoUrl ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-amber-50 to-amber-100 flex items-center justify-center transition-transform duration-500 group-hover:scale-110">
              <span className="text-6xl font-black text-amber-300">{plan.name.charAt(0)}</span>
            </div>
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-amber-100/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          {plan.isHighlight && (
        ...
        <div className="p-4 flex flex-col flex-1">
          ...
          {badges.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-3">
              {badges.slice(0, 4).map((b: string, i: number) => (
                <span key={i} className="text-[10px] px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 border border-amber-200">{b}</span>
              ))}
            </div>
          )}

          <div className="mb-3">
            <div className="text-3xl font-black text-amber-600">{Number(plan.dailyRate).toFixed(2)}%</div>
          ...
          <div className="space-y-1.5 mb-3 text-xs flex-1">
            ...
            {plan.totalReturn && (
              <div className="flex justify-between"><span className="text-muted-foreground">Total Return</span><span className="text-amber-600 font-bold">{Number(plan.totalReturn).toFixed(0)}%</span></div>
            )}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-amber-600 font-medium uppercase tracking-widest">Premium Collection</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-black text-foreground mb-2">
            🏆 <span className="text-amber-600">Golden</span> Collection
          </h1>
          ...
          <div className="flex items-center gap-6 mt-6 pt-6 border-t border-border/40">
            <div><div className="text-lg font-bold text-amber-600">{plans.length}</div><div className="text-xs text-muted-foreground">Total Plans</div></div>
        ...
        {/* CBAG 필수 포함 배너 */}
        <div className="mb-6 p-4 rounded-xl bg-gradient-to-r from-cyan-500/10 via-blue-500/10 to-cyan-500/10 border border-cyan-500/30 flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-cyan-500/20 flex items-center justify-center flex-shrink-0">
            <span className="text-lg">🛡️</span>
          </div>
          <div className="flex-1">
            <div className="text-sm font-bold text-cyan-300">C-BAG Insurance 필수 포함</div>
            <div className="text-xs text-gray-400 mt-0.5">Golden Collection 투자 시 CBAG 보험 콜렉션을 추가할 수 있습니다. 투자금의 일부를 보험으로 운용하여 리스크를 분산하세요.</div>
          </div>
          <div className="text-xs text-cyan-400 font-semibold bg-cyan-500/10 px-2 py-1 rounded-lg border border-cyan-500/30 flex-shrink-0">CBAG</div>
        </div>
        ...
        {!isLoading && plans.length === 0 && (
          <div className="text-center py-20"><div className="text-4xl mb-4">🏆</div><div className="text-muted-foreground text-sm">No Golden plans available</div></div>
        )}
      </div>
