...
function PlanCard({ plan }: { plan: any }) {
  ...
  return (
    <Link href={`/plan/${plan.id}`}>
      <div className="relative bg-[#0d0d0d] border border-amber-500/20 rounded-xl overflow-hidden cursor-pointer transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-amber-500/40 hover:border-amber-400/60 group flex flex-col">
        {/* 카드 이미지 영역 */}
        <div className="relative h-40 overflow-hidden">
          {plan.logoUrl ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-amber-900/40 to-amber-600/20 flex items-center justify-center transition-transform duration-500 group-hover:scale-110">
              <span className="text-6xl font-black text-amber-400/20">{plan.name.charAt(0)}</span>
            </div>
          )}
          ...
          {plan.isHighlight && (
            <div className="absolute top-2 right-2">
              <Badge className="bg-amber-500/90 text-black border-0 text-[10px] font-bold">HOT</Badge>
            </div>
          )}
          {/* 상단 하이라이트 라인 */}
          {plan.isHighlight && (
        ...
        <div className="p-4 flex flex-col flex-1">
          ...
          <div className="space-y-1.5 mb-3 text-xs flex-1">
            ...
            {plan.totalReturn && (
              <div className="flex justify-between"><span className="text-gray-500">Total Return</span><span className="text-amber-400 font-bold">{Number(plan.totalReturn).toFixed(0)}%</span></div>
            )}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="mb-10">
          ...
          <div className="flex items-center gap-6 mt-6 pt-6 border-t border-white/5">
            <div><div className="text-lg font-bold text-amber-400">{plans.length}</div><div className="text-xs text-gray-500">Total Plans</div></div>
        ...
        {!isLoading && plans.length === 0 && (
          <div className="text-center py-20"><div className="text-4xl mb-4">🏆</div><div className="text-gray-400 text-sm">No Golden plans available</div></div>
        )}
      </div>
