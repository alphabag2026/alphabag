...
function TreeNodeItem({ node, depth = 0 }: { node: ReferralNode; depth?: number }) {
...
function ReferralTreeView() {
  ...
  const [nodeDetailId, setNodeDetailId] = useState<number | null>(null);
  ...
  const { data: nodeDetailData, isLoading: nodeDetailLoading } = trpc.users.detail.useQuery(
    { userId: nodeDetailId ?? 0 },
    { enabled: !!nodeDetailId && nodeDetailOpen }
  );
    ...
    <>
            ...
            <CardContent className="pt-0">
              ...
              {selectedUserId && !recursiveLoading && recursiveTreeData && (
                <ReferralD3Tree
                  ...
                  width={580}
                  height={500}
                  onNodeClick={(id) => { setNodeDetailId(id); setNodeDetailOpen(true); }}
                />
              )}
            </CardContent>
          </Card>
      ...
      <Dialog open={nodeDetailOpen} onOpenChange={setNodeDetailOpen}>
        <DialogContent className="bg-card border-border max-w-lg max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-sm">사용자 상세 정보</DialogTitle>
          </DialogHeader>
          {nodeDetailLoading && (
          ...
          {nodeDetailData && (
            <div className="space-y-4 text-sm">
              <div className="space-y-2">
                <h3 className="font-medium text-xs text-muted-foreground uppercase tracking-wide">기본 정보</h3>
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-muted/30 rounded-lg p-2">
                    <p className="text-xs text-muted-foreground">이름</p>
                    <p className="font-medium truncate">{(nodeDetailData.user as any)?.name ?? '—'}</p>
                  </div>
                  <div className="bg-muted/30 rounded-lg p-2">
                    <p className="text-xs text-muted-foreground">추천 코드</p>
                    <p className="font-mono text-primary">{(nodeDetailData.user as any)?.referralCode ?? '—'}</p>
                  </div>
                  <div className="bg-muted/30 rounded-lg p-2 col-span-2">
                    <p className="text-xs text-muted-foreground">지갑 주소</p>
                    <p className="font-mono text-xs truncate">{(nodeDetailData.user as any)?.walletAddress ?? '—'}</p>
                  </div>
                </div>
              </div>
              {nodeDetailData.investments && nodeDetailData.investments.length > 0 && (
                <div className="space-y-2">
                  <h3 className="font-medium text-xs text-muted-foreground uppercase tracking-wide">투자 내역 ({nodeDetailData.investments.length}건)</h3>
                  <div className="space-y-1 max-h-40 overflow-y-auto">
                    {nodeDetailData.investments.map((inv: any) => (
                      <div key={inv.id} className="flex items-center justify-between bg-muted/20 rounded px-2 py-1.5">
                        <span className="text-xs text-muted-foreground">{inv.planName ?? `Plan #${inv.planId}`}</span>
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-medium">${Number(inv.amount).toLocaleString()}</span>
                          <Badge variant={inv.status === 'active' ? 'default' : 'secondary'} className="text-xs px-1 py-0 h-4">
                            {inv.status}
                          </Badge>
                        </div>
                      </div>
              ...
              {nodeDetailData.referrals && nodeDetailData.referrals.length > 0 && (
                <div className="space-y-2">
                  <h3 className="font-medium text-xs text-muted-foreground uppercase tracking-wide">직접 추천 ({nodeDetailData.referrals.length}명)</h3>
                  <div className="space-y-1 max-h-32 overflow-y-auto">
                    {nodeDetailData.referrals.map((ref: any) => (
                      <div key={ref.id} className="flex items-center justify-between bg-muted/20 rounded px-2 py-1.5">
                        <span className="font-mono text-xs text-muted-foreground">{ref.refereeId ? `User #${ref.refereeId}` : '—'}</span>
                        <button
                          className="text-xs text-primary hover:underline"
                          onClick={() => { setNodeDetailId(ref.refereeId); }}
                        >
                          상세 보기
                        </button>
                      </div>
