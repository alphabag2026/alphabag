...
import { Search, Download, UserCheck, UserX, Shield, ChevronLeft, ChevronRight, Eye } from "lucide-react";
...
export default function Users() {
  ...
  const downloadCSV = () => {
    if (!data?.data) return;
    const headers = ["ID", "Name", "Email", "Wallet", "Referral Code", "Referred By", "KYC", "Role", "Total Invested", "Joined"];
    ...
    <AdminLayout title="Users & Organization">
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <Input
              ...
              placeholder="Search by wallet, referral code, name..."
                  ...
                  <tr key={user.id}>
                    ...
                    <td>
                      <div>
                        ...
                        {user.referredBy && <p className="text-xs text-muted-foreground">by: {user.referredBy}</p>}
                      </div>
                    </td>
                    <td>
      ...
      <Dialog open={kycDialogOpen} onOpenChange={setKycDialogOpen}>
        <DialogContent className="bg-card border-border max-w-sm">
          ...
          <div className="py-2">
            <p className="text-sm text-muted-foreground mb-3">User: <span className="text-foreground font-medium">{selectedUser?.name ?? `#${selectedUser?.id}`}</span></p>
