    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-400 text-sm">밈토큰 · 고수익 · 커뮤니티 드리븐</p>
        </div>
            ...
            {sorted.map((plan: any) => {
              ...
              return (
                <div
                  ...
                  className="relative bg-[#0d0d0d] border border-pink-500/30 rounded-xl overflow-hidden cursor-pointer transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-pink-500/40 group"
                >
                  <div className="relative h-36 overflow-hidden">
                    {plan.logoUrl ? (
                      ...
                      <div className="w-full h-full bg-gradient-to-br from-pink-900/40 to-pink-600/20 flex items-center justify-center transition-transform duration-500 group-hover:scale-110">
                        <span className="text-5xl font-black opacity-30 text-pink-400">{plan.name.charAt(0)}</span>
                      </div>
                    )}
                  ...
                  <div className="p-4">
                    ...
                    {badges.length > 0 && (
                      <div className="flex flex-wrap gap-1 mb-3">
                        {badges.slice(0, 3).map((b: string, i: number) => (
                          <span key={i} className="text-[10px] px-2 py-0.5 rounded-full border bg-pink-500/20 text-pink-300 border-pink-500/30">{b}</span>
                        ))}
                      </div>
                    )}
                    <div className="mb-3">
                      <div className="text-2xl font-bold text-pink-400">{Number(plan.dailyRate).toFixed(2)}%</div>
                    ...
                    <button className="w-full py-2 rounded-lg text-xs font-semibold transition-all duration-200 border bg-pink-500/20 text-pink-300 border-pink-500/30 hover:bg-pink-500/30">
                      View Details →
                    </button>
                  </div>
                </div>
