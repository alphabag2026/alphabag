...
function NodeCard({ plan }: { plan: any }) {
  ...
  return (
    <div className="relative bg-[#0d0d0d] border border-purple-500/20 hover:border-purple-400/50 rounded-xl p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-purple-500/10 flex flex-col">
      {plan.isHighlight && (
        <div className="absolute top-0 left-0 right-0 h-[2px] rounded-t-xl bg-gradient-to-r from-transparent via-purple-400 to-transparent" />
      )}

      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-xl bg-purple-500/20 flex items-center justify-center text-purple-400 font-bold text-lg flex-shrink-0">
          {plan.logoUrl
        ...
        {plan.isHighlight && (
          <Badge className="bg-purple-500/20 text-purple-300 border border-purple-500/30 text-[10px] flex-shrink-0">HOT</Badge>
        )}
      </div>
      ...
      <div className="space-y-2 mb-4 text-xs flex-1">
        ...
        {plan.totalReturn && (
          <div className="flex justify-between">
            ...
            <span className="text-purple-400 font-bold">{Number(plan.totalReturn).toFixed(0)}%</span>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-1 mb-4">
        {[1, 2, 3, 4, 5].map((s) => (
          <Star key={s} className={`w-3 h-3 ${s <= Math.round(rating) ? "text-purple-400 fill-purple-400" : "text-gray-600"}`} />
        ))}
      ...
      <button
        ...
        className="w-full py-2.5 rounded-xl bg-purple-500/20 text-purple-300 border border-purple-500/30 hover:bg-purple-500/30 text-sm font-semibold transition-all"
      >
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="mb-10">
          ...
          <div className="flex items-center gap-6 mt-6 pt-6 border-t border-white/5">
            <div>
              <div className="text-lg font-bold text-purple-400">{plans.length}</div>
            ...
            <div>
              <div className="text-lg font-bold text-white">{plans.filter((p: any) => p.isHighlight).length}</div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {plans.map((plan: any) => (
              <NodeCard key={plan.id} plan={plan} />
            ))}
          </div>
        )}
        ...
        {!isLoading && plans.length === 0 && (
          <div className="text-center py-20">
            ...
            <div className="text-gray-400 text-sm">No Node products available</div>
          </div>
        )}
      </div>
