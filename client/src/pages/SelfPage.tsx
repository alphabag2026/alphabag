...
function SelfCard({ plan }: { plan: any }) {
  ...
  return (
    <div className="relative bg-[#0d0d0d] border border-blue-500/20 hover:border-blue-400/50 rounded-xl p-5 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-blue-500/10 flex flex-col">
      {plan.isHighlight && (
        <div className="absolute top-0 left-0 right-0 h-[2px] rounded-t-xl bg-gradient-to-r from-transparent via-blue-400 to-transparent" />
      )}

      <div className="flex items-center gap-3 mb-4">
        <div className="w-11 h-11 rounded-xl bg-blue-500/20 flex items-center justify-center text-blue-400 font-bold text-lg flex-shrink-0">
          {plan.logoUrl
        ...
        {plan.isHighlight && (
          <Badge className="bg-blue-500/20 text-blue-300 border border-blue-500/30 text-[10px] flex-shrink-0">HOT</Badge>
        )}
      </div>
      ...
      <div className="space-y-2 mb-4 text-xs flex-1">
        ...
        {plan.totalReturn && (
          <div className="flex justify-between">
            ...
            <span className="text-blue-400 font-bold">{Number(plan.totalReturn).toFixed(0)}%</span>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-1 mb-4">
        {[1, 2, 3, 4, 5].map((s) => (
          <Star key={s} className={`w-3 h-3 ${s <= Math.round(rating) ? "text-blue-400 fill-blue-400" : "text-gray-600"}`} />
        ))}
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-xs text-blue-400 font-medium uppercase tracking-widest">Custom Strategy</span>
          </div>
          <h1 className="text-3xl md:text-4xl font-black text-white mb-2">
            ⚡ <span className="text-blue-400">Self</span> Collection
          </h1>
          <p className="text-gray-400 text-sm max-w-xl">
            Custom Strategy · Flexible · Self-managed investment plans tailored to your goals.
          </p>

          <div className="flex items-center gap-6 mt-6 pt-6 border-t border-white/5">
            <div>
              <div className="text-lg font-bold text-blue-400">{plans.length}</div>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {plans.map((plan: any) => (
              <SelfCard key={plan.id} plan={plan} />
            ))}
          </div>
        )}
        ...
        {!isLoading && plans.length === 0 && (
          <div className="text-center py-20">
            ...
            <div className="text-gray-400 text-sm">No Self plans available</div>
          </div>
        )}
      </div>
